...
场景说明：从历史 checkpoint 恢复训练并继续记录结果。
...
if __name__ == "__main__":
    ...
    pipeline = (
        ExperimentPipeline(
            results_root="./results_templates/resume_from_checkpoint",
            base_config={'seed': 42, 'task': 'resume_from_checkpoint'},
        )
        .grid({'lr': [0.001], 'resume_epoch': [0, 5, 10]})
        .variants([{'model': 'tiny'}])
        .exclude_completed()
    )
